# HANDOFF — 2026-06-02 (T-101 MERGED #9; T-100/T-110 EventStore DONE+REVIEWED; T-111 Q-020 precursor DONE [decision→RED→GREEN, 340 green], tip af74ea2; NEXT: the T-111 Scorer reducer proper)
...
- **Worktree:** none. **In flight (2026-06-02):** NOTHING running; clean tree; tip `af74ea2`, all pushed. T-110 review chain complete (`a572c88` + `020dffb`). **T-111's Q-020 precursor is DONE end-to-end:** decision (`48d6f53`, Option D via architect→critic) → RED (`bfa7ecf`, 5 failing tests) → GREEN (`af74ea2`, framing implemented). `event_type` is now a framed hash input + first-class yielded key — tamper-evident, on the read surface, `payload` unchanged. Full suite **340 passed / 6 skipped**, ruff clean. **The remaining T-111 work is the Scorer reducer PROPER** (3-pillar grading) — see step 2.
...
2. **T-111 — Scorer** (CRITICAL, NEXT BUILD). ✅ **Q-020 precursor DONE** (`48d6f53`→`bfa7ecf`→`af74ea2`): `event_type` is now framed into `row_hash` (Option D, ADR-0007 Addendum 1) + a first-class yielded key, so the reducer can dispatch on `row["event_type"]` and a tampered discriminator trips `verify_chain`. The mandated negative + positive-discrimination + key-set tests are green. **REMAINING = the Scorer reducer PROPER:**
   - **Build the 3-pillar grading reducer** (ATTACK / DETECT-F1-three-window / MITIGATE-F2-functional-path), `idempotency_key`-keyed (`(scenario_id, challenge_id, pillar, manifest_hash)`), per **ADR-0001** (manifest-as-oracle, event-sourced). `tester` RED first → `implementer`.
   - It **folds the EventStore log** (`store.fold(reducer, init)` / `replay_from`), dispatching on the now-trustworthy `(event_type, version)` (ADR §5). `fold`/`replay_from` yield persisted dicts (ADR-0007 §1a) — `contracts.load_<shape>()` for typed views.
   - **Residual to honor (from the Q-020 critic pass):** Option D authenticates `event_type` *immutability*, NOT its *correspondence to the payload it labels*. If the reducer must trust that `event_type` matches its payload shape, add an Option-C-style cross-check (column vs payload-derived shape → ungradeable on mismatch). Decide at reducer-design time; recorded in ADR-0007 Addendum 1 Consequences + [[Q-020]].
   - **Apply the UNGRADEABLE fold rule** (ADR-0001 / M4): any un-terminated `correlation_id` folds to UNGRADEABLE; resume is idempotent.
