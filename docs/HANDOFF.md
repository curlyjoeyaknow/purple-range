# HANDOFF — 2026-06-02 (T-101 MERGED #9; T-100/ADR-0007 + T-110 EventStore DONE+GREEN+REVIEWED, tip 020dffb; NEXT: T-111 Scorer, opening with the Q-020 decision)
...
- **Worktree:** none. **In flight (2026-06-02):** T-110 review chain complete (internal `a572c88` + external `020dffb`); branch tip is `020dffb`, all pushed, clean tree. About to open **T-111** with the **Q-020 architect+critic decision** (default Option D, frame-it). No subagents currently running.
...
2. **T-111 — Scorer** (CRITICAL, NEXT BUILD — **IN FLIGHT: about to run the Q-020 architect+critic**). **FIRST resolve [[Q-020]]** (`docs/OPEN-QUESTIONS.md`, 🔴 from the T-110 review): how the reducer gets each event's type discriminator **tamper-evidently** — `event_type` is currently an UNHASHED column (tampering it keeps `verify_chain` True), absent from the yielded dict, yet ADR §5 dispatches on it. Decide with `architect` + `critic`. **Default is now Option D (frame-it)** — preferred over the old Option A (additive payload field) on oracle-preservation grounds; land via a short ADR-0007 addendum. **Whichever wins MUST add a negative test `test_verify_chain_detects_tampered_event_type`** (the suite currently has NO test that tampers a non-`payload` column — exactly how this slipped through). THEN build: 3-pillar grading (ATTACK / DETECT-F1-three-window / MITIGATE-F2-functional-path), `idempotency_key`-keyed, per **ADR-0001**, `tester` RED first → `implementer`. Folds the EventStore log; `fold`/`replay_from` yield persisted dicts (ADR-0007 §1a) — `contracts.load_<shape>()` for typed views.
