...
1. ✅ **T-110 cheap-fix pass DONE** (`a572c88`): `SqliteEventStore.close()` + `__enter__`/`__exit__`, hoisted `import json`, `append([])` no-op guard — ruff clean, 335 passed. Two NITs deliberately deferred (tracked in RED-TEAM.md): `replay_from(seq<=0)` doc, and renaming `test_scenario_aborted_is_idempotent_on_correlation_id` → `..._reducer_idempotent...`. **Remaining before T-111:** `external-reviewer` on the T-110 diff (fresh-context read of the framing/verify code + independent take on the [[Q-020]] event_type finding) — may instead be folded into GATE A.
