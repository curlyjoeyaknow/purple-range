...
1. **T-110 cheap-fix pass** (agent: `implementer`, effort S — apply then re-run `ruff check .` + `pytest tests/ -q`): add `SqliteEventStore.close()` + `__enter__`/`__exit__` (🟠 connection leak); hoist the function-local `import json` in `event_store.py` `_iter_rows` to module top; early-return no-op guard for `append([])`; guard/doc `replay_from(seq<=0)`; rename `test_scenario_aborted_is_idempotent_on_correlation_id` → `..._reducer_idempotent...` (store correctly doesn't dedup). Then `external-reviewer` on the diff.
2. **T-111 — Scorer** (CRITICAL, NEXT BUILD). **FIRST resolve [[Q-020]]** (`docs/OPEN-QUESTIONS.md`, 🔴 from the T-110 review): how the reducer gets each event's type discriminator **tamper-evidently** — `event_type` is currently an UNHASHED column (tampering it keeps `verify_chain` True), absent from the yielded dict, yet ADR §5 dispatches on it. Decide with `architect` + `critic` (default Option A: additive `event_type` field into the hashed payload via a short ADR-0007 addendum). THEN build: 3-pillar grading (ATTACK / DETECT-F1-three-window / MITIGATE-F2-functional-path), `idempotency_key`-keyed, per **ADR-0001**, `tester` RED first → `implementer`. Folds the EventStore log; `fold`/`replay_from` yield persisted dicts (ADR-0007 §1a) — `contracts.load_<shape>()` for typed views.
